...
{
    constexpr size_t step = sizeof(__m256) / sizeof(float);
    ...
    if constexpr (is_same_v<Ts, float>) {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; x += step) {
                __m256 v = cvtepuX_ps<__m256, Ts>(s + x);
                store<__m256>(d + x, v);
            }
            s += spitch;
            d += dpitch;
    ...
    const int width, const int radius, const float* weights, const int remains)
{
    ...
    for (int x = 0; x < width; x += step * 2) {
        ...
        __m256 sum00 = zero<__m256>(); __m256 sum10 = zero<__m256>();
        __m256 sum20 = zero<__m256>(); __m256 sum30 = zero<__m256>();
        __m256 sum01 = zero<__m256>(); __m256 sum11 = zero<__m256>();
        __m256 sum21 = zero<__m256>(); __m256 sum31 = zero<__m256>();

        for (int v = -radius; v <= radius; ++v) {
            __m256 k = set1_ps<__m256>(weights[v]);
            sum00 = fmadd<__m256>(k, loadu<__m256>(s0 + x + v), sum00);
            sum01 = fmadd<__m256>(k, loadu<__m256>(s0 + x + v + step), sum01);
            sum10 = fmadd<__m256>(k, loadu<__m256>(s1 + x + v), sum10);
            sum11 = fmadd<__m256>(k, loadu<__m256>(s1 + x + v + step), sum11);
            sum20 = fmadd<__m256>(k, loadu<__m256>(s2 + x + v), sum20);
            sum21 = fmadd<__m256>(k, loadu<__m256>(s2 + x + v + step), sum21);
            sum30 = fmadd<__m256>(k, loadu<__m256>(s3 + x + v), sum30);
            sum31 = fmadd<__m256>(k, loadu<__m256>(s3 + x + v + step), sum31);
        }
        if constexpr (is_same_v<Td, float>) {
            store<__m256>(d0 + x, sum00);
            store<__m256>(d0 + x + step, sum01);
            if (remains < 2) continue;
            store<__m256>(d1 + x, sum10);
            store<__m256>(d1 + x + step, sum11);
            if (remains < 3) continue;
            store<__m256>(d2 + x, sum20);
            store<__m256>(d2 + x + step, sum21);
            if (remains < 4) continue;
            store<__m256>(d3 + x, sum30);
            store<__m256>(d3 + x + step, sum31);
        }
        else if constexpr (is_same_v<Td, uint16_t>) {
            __m256i data = cvtps_epu16<__m256i, __m256>(sum00, sum01);
            store<__m256i>(d0 + x, data);
            if (remains < 2) continue;
            data = cvtps_epu16<__m256i, __m256>(sum10, sum11);
            store<__m256i>(d1 + x, data);
            if (remains < 3) continue;
            data = cvtps_epu16<__m256i, __m256>(sum20, sum21);
            store<__m256i>(d2 + x, data);
            if (remains < 4) continue;
            data = cvtps_epu16<__m256i, __m256>(sum30, sum31);
            store<__m256i>(d3 + x, data);
        }
        else if constexpr (is_same_v<Td, uint8_t>) {
            __m128i data = cvtps_epu8<__m128i, __m256>(sum00, sum01);
            store<__m128i>(d0 + x, data);
            if (remains < 2) continue;
            data = cvtps_epu8<__m128i, __m256>(sum10, sum11);
            store<__m128i>(d1 + x, data);
            if (remains < 3) continue;
            data = cvtps_epu8<__m128i, __m256>(sum20, sum21);
            store<__m128i>(d2 + x, data);
            if (remains < 4) continue;
            data = cvtps_epu8<__m128i, __m256>(sum30, sum31);
    ...
    const float)
{
    ...
    for (int y = 0; y < height; y += 4) {
        for (int x = 0; x < width; x += step) {
            __m256 k0 = set1_ps<__m256>(weights[0]);
            __m256 val = cvtepuX_ps<__m256, Ts>(ptr[0] + x);
            __m256 sum0 = fmul<__m256>(k0, val);

            __m256 k1 = set1_ps<__m256>(weights[1]);
            val = cvtepuX_ps<__m256, Ts>(ptr[1] + x);
            sum0 = fmadd<__m256>(k1, val, sum0);
            __m256 sum1 = fmul<__m256>(k0, val);

            __m256 k2 = set1_ps<__m256>(weights[2]);
            val = cvtepuX_ps<__m256, Ts>(ptr[2] + x);
            sum0 = fmadd<__m256>(k2, val, sum0);
            sum1 = fmadd<__m256>(k1, val, sum1);
            __m256 sum2 = fmul<__m256>(k0, val);

            if constexpr (RADIUS == 1) {
                store<__m256>(hb[0] + x, sum0);

                val = cvtepuX_ps<__m256, Ts>(ptr[3] + x);
                sum1 = fmadd<__m256>(k2, val, sum1);
                sum2 = fmadd<__m256>(k1, val, sum2);
                __m256 sum3 = fmul<__m256>(k0, val);
                store<__m256>(hb[1] + x, sum1);

                val = cvtepuX_ps<__m256, Ts>(ptr[4] + x);
                sum2 = fmadd<__m256>(k2, val, sum2);
                sum3 = fmadd<__m256>(k1, val, sum3);
                store<__m256>(hb[2] + x, sum2);

                val = cvtepuX_ps<__m256, Ts>(ptr[5] + x);
                sum3 = fmadd<__m256>(k2, val, sum3);
                store<__m256>(hb[3] + x, sum3);

            } else {
                __m256 k3 = set1_ps<__m256>(weights[3]);
                val = cvtepuX_ps<__m256, Ts>(ptr[3] + x);
                sum0 = fmadd<__m256>(k3, val, sum0);
                sum1 = fmadd<__m256>(k2, val, sum1);
                sum2 = fmadd<__m256>(k1, val, sum2);
                __m256 sum3 = fmul<__m256>(k0, val);
                ...
                for (int v = 4; v < length; ++v) {
                    ...
                    k1 = k2;
                    k2 = k3;
                    k3 = set1_ps<__m256>(weights[v]);
                    val = cvtepuX_ps<__m256, Ts>(ptr[v] + x);
                    sum0 = fmadd<__m256>(k3, val, sum0);
                    sum1 = fmadd<__m256>(k2, val, sum1);
                    sum2 = fmadd<__m256>(k1, val, sum2);
                    sum3 = fmadd<__m256>(k0, val, sum3);
                }
                store<__m256>(hb[0] + x, sum0);

                val = cvtepuX_ps<__m256, Ts>(ptr[length] + x);
                sum1 = fmadd<__m256>(k3, val, sum1);
                sum2 = fmadd<__m256>(k2, val, sum2);
                sum3 = fmadd<__m256>(k1, val, sum3);
                store<__m256>(hb[1] + x, sum1);

                val = cvtepuX_ps<__m256, Ts>(ptr[length + 1] + x);
                sum2 = fmadd<__m256>(k3, val, sum2);
                sum3 = fmadd<__m256>(k2, val, sum3);
                store<__m256>(hb[2] + x, sum2);

                val = cvtepuX_ps<__m256, Ts>(ptr[length + 2] + x);
                sum3 = fmadd<__m256>(k3, val, sum3);
                store<__m256>(hb[3] + x, sum3);
            }
        }
