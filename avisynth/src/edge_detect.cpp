...
void __stdcall TCannyM::standerd_operator(int width, int height)
{
...
void __stdcall TCannyM::sobel_operator(int width, int height)
{
    const float* srcp = blur_frame;
    float* dstp = edge_mask;
    uint8_t *dir = direction;
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x += 16) {
            __m128i d[4];
            for (int i = 0; i < 4; i++) {
                __m128 gx = _mm_sub_ps(_mm_loadu_ps(p0 + x + 1 + i * 4),
                                       _mm_loadu_ps(p2 + x - 1 + i * 4));
                __m128 gy = gx;
                __m128 xmm0 = _mm_loadu_ps(p0 + x - 1 + i * 4);
                gx = _mm_sub_ps(gx, xmm0);
                gy = _mm_add_ps(gy, xmm0);
                xmm0 = _mm_loadu_ps(p2 + x + 1 + i * 4);
                gx = _mm_add_ps(gx, xmm0);
                gy = _mm_sub_ps(gy, xmm0);
                gx = _mm_sub_ps(gx, _mm_mul_ps(two, _mm_loadu_ps(p1 + x - 1 + i * 4)));
                gx = _mm_add_ps(gx, _mm_mul_ps(two, _mm_loadu_ps(p1 + x + 1 + i * 4)));
                gy = _mm_add_ps(gy, _mm_mul_ps(two, _mm_load_ps(p0 + x + i * 4)));
                gy = _mm_sub_ps(gy, _mm_mul_ps(two, _mm_load_ps(p2 + x + i * 4)));
